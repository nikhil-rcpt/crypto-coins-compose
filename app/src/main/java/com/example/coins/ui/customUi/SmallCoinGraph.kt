...
fun getMinMax(data: List<Int>) : Pair<Int,Int>{
...
fun createPoints(data: List<Int>, size: Size?): List<PointF> {
    size ?: return listOf()
...
private fun isGain(data: List<Int>) : Boolean {
    return data.last()>data.first()
}

private fun calculateConnectionPointsForBezierCurve(points : List<PointF>): Triple<List<PointF>, List<PointF>, List<PointF>> {
...
@Composable
fun CoinGraph(dataSet : List<Int> = listOf(10,11,15,9,10,12),styleThin:Boolean = false) {
    ...
    val isGain by remember { mutableStateOf(isGain(dataSet)) }
    ...
    var points by remember { mutableStateOf(createPoints(dataSet,size)  ) }
    var data by remember { mutableStateOf( calculateConnectionPointsForBezierCurve(points)) }
    var finalData by remember { mutableStateOf(drawBezierCurve(data.first,data.second,data.third,size)) }
    val strokeWidth by remember { mutableStateOf(if(styleThin) 5f else 5F )}
    Canvas(modifier= Modifier.fillMaxSize()){
        ...
        if(size!=this.size) {
            size=this.size
            points=createPoints(dataSet,size);
            data=calculateConnectionPointsForBezierCurve(points)
            finalData=drawBezierCurve(data.first,data.second,data.third,size)
        }
    }
}

private fun drawBezierCurve(points : List<PointF>, conPoint1 : List<PointF>
